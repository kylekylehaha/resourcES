...
def Status(Name):
    ...
    * Required information:
    ...
    if infotype == 'borrow':
        print("borrow")
        cursor.execute('SELECT Order_num, Ename, Renewal_limit, Loan_period, Notice, R.Enum, Ephoto, Order_status, Rank, Renewal_times, Due_date FROM RESOURCES AS R JOIN BORROW AS B ON R.Enum=B.Enum WHERE B.Ssn=%s',ssn)
        data = cursor.fetchall()
        #print(data)
